...
from restaurant.tripadvisor.tripadvisor_api_handler import TripadvisorFullContent, TripadvisorLocationDetailsHandler
...
def parse_tripadvisor_price_level(price_level: str) -> List[PriceLevel]:
    ...
    :return:
    """
    return list(map(trip_advisor_price_level_mapper.get, map(str.strip, price_level.split("-"))))
...
class TripadvisorFeatures:
    ...
    CARD_PAYMENT_OPTIONS = ["American Express", "Accepts Credit Cards", "Mastercard", "Visa", "Digital Payments"]
    ...
    def __init__(self, trip_place: TripadvisorLocationDetailsHandler):
        ...
        self.is_accessible = any(accessibility_option in trip_place.features for accessibility_option in self.TYPES_OF_ACCESSIBILITY)
        self.is_credit_card_accepted = any(card_option in trip_place.features for card_option in self.CARD_PAYMENT_OPTIONS)
        self.is_alcohol_served = any(alcohol_option in trip_place.features for alcohol_option in self.ALCOHOL_SERVING_OPTIONS)
        self.wifi = any(wifi_option in trip_place.features for wifi_option in self.WIFI_OPTIONS)
        self.is_reservable = any(reservable_option in trip_place.features for reservable_option in self.RESERVABLE_OPTIONS)
